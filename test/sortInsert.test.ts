...
describe('sortInsert', () => {
  ...
  describe('asynchronous mode', () => {
    test('should insert in the beginning and return the return value of the generator if no return value specified', async () => {
      const result = yieldReturnValue(sortInsert(-1, (a, b) => a - b)(toAsync(range(0, 10, 2, 100))))
      expect(await toArray(result)).toEqual([-1, 0, 2, 4, 6, 8, 100]);
    });
    test('should insert in the middle and return the return value of the generator if no return value specified', async () => {
      const result = yieldReturnValue(sortInsert(5, (a, b) => a - b)(toAsync(range(0, 10, 2, 100))))
      expect(await toArray(result)).toEqual([0, 2, 4, 5, 6, 8, 100]);
    });
    test('should insert in the end and return the return value of the generator if no return value specified', async () => {
      const result = yieldReturnValue(sortInsert(9, (a, b) => a - b)(toAsync(range(0, 10, 2, 100))))
      expect(await toArray(result)).toEqual([0, 2, 4, 6, 8, 9, 100]);
    });
    test('should pass down the `next` value, preserving the feed', async () => {
      const result = feed(toAsync(range(1, 10, 2)))(sortInsert(9, (a, b) => a - b)(toAsync(feedMap((x: number) => x * 2, 0))))
      expect(await toArray(result)).toEqual([0, 2, 6, 9, 10, 14]);
    });
  });
});