...
describe('headWhile', () => {
  ...
  test('should get the first elements until the index is === 5', () => {
    const result = headWhile((_: number, i: number) => i < 5)(range(10, 20))
    expect([...result]).toEqual([10, 11, 12, 13, 14]);
  });
  test('should get the first elements until the `next` is === 5', () => {
    const result = pipe(
      headWhile<number>((_1: number, _2: number, next: number) => !next || next < 10),
      feed<number>(range(1)),
