...
describe('mixAll', () => {
  test('should mix two generators', () => {
    const result = mixAll<number | string>(fromArray(['a', 'b', 'c']), range(0, 4))
    expect([...result]).toEqual([['a', 0], ['b', 1], ['c', 2], [undefined, 3]]);
  });
  test('should mix three generators', () => {
    const result = mixAll<number | string>(fromArray(['a', 'b', 'c']), range(0, 4), fromArray([1, 1, 2, 5, 7]))
  ...
  test('should pass down the `next` value to the two generators', () => {
    const gen = mixAll<number | string>(feedMap((x: number) => x * 2, 0), feedMap((x: number) => x + 1, 0))
    const result = pipe(
      feed(range(1, 10))
    )(gen)
  ...
  test('should return the return value of each generator at the end', () => {
    const result = yieldReturnValue(mixAll<number | string>(range(0, 5, 1, 100), range(0, 8, 1, 200)))
