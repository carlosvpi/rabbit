...
describe('mixRace', () => {
  ...
  test('should pass down the `next` value to the two generators', () => {
    const gen = mixRace<number | string>(feedMap((x: number) => x * 2, 0), feedMap((x: number) => x + 1, 0))
    const result = pipe(
      feed(range(1, 10))
    )(gen)
  ...
  test('should return the return value of all ended generators as soon as they are done', () => {
    const result = yieldReturnValue(mixRace<number | string>(range(0, 5, 1, 100), range(0, 5, 1, 150), range(0, 8, 1, 200)))
