...
describe('takeWhile', () => {
  ...
  describe('asynchronous mode', () => {
    test('should get the first elements until one is no less than 5', async () => {
      const result = takeWhile((x: number) => x < 5)(toAsync(range(0, 10)))
      expect(await toArray(result)).toEqual([0, 1, 2, 3, 4]);
    });
    test('should get the first elements until the index is === 5', async () => {
      const result = takeWhile((_: number, i: number) => i < 5)(toAsync(range(10, 20)))
      expect(await toArray(result)).toEqual([10, 11, 12, 13, 14]);
    });
    ...
    test('should return the specified return value argument if g does not run out', async () => {
      const result = yieldReturnValue(takeWhile((x: number) => x < 5, 100)(toAsync(range(0, 10))))
      expect(await toArray(result)).toEqual([0, 1, 2, 3, 4, 100]);
    });
    test('should return the specified return value argument even if g does run out with its own return value', async () => {
      const result = yieldReturnValue(takeWhile((x: number) => x < 5, 100)(toAsync(range(0, 10, 1, 200))))
      expect(await toArray(result)).toEqual([0, 1, 2, 3, 4, 100]);
    });
