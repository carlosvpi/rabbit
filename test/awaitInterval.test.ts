...
describe('awaitInterval', () => {
  test('should awaitInterval 5 elements to our generator', async () => {
    const result = drop<number>(1)((map(([a, b]: number[]) => b - a) as (_: AsyncGenerator<number>) => AsyncGenerator<number>)(last<number>(2)(awaitInterval(10, (stop, _, i) => {i === 10 && stop()}))))
  ...
  test('should awaitInterval 5 elements to our generator and return what is passed to "stop"', async () => {
    const result = await returns(awaitInterval(10, (stop, _, i) => {i === 10 && stop(100)}))
